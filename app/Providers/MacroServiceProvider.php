...
class MacroServiceProvider extends ServiceProvider
{
    ...
    public function boot()
    {
        File::macro('streamUpload', function($path, $fileName, $file, $overWrite = true) {
            ...
            $config = Config::get('filesystems.disks.s3.videos');
            $client = new S3Client([
                'credentials' => [
                    'key'    => $config['key'],
        ...
        File::macro('fileUpload', function($path, $fileName, $resource, $overWrite = true) {
            // Set up S3 connection.
            // $resource = $file;
            $config = Config::get('filesystems.disks.s3.assets');
            $client = new S3Client([
                'credentials' => [
                    'key'    => $config['key'],
                    'secret' => $config['secret'],
